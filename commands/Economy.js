...
async function work (event, api) {
    ...
    if (global.data.jacoin[event.threadID][event.senderID].workcountdown > time.getTime()) {
        api.sendMessage(`Bạn phải đợi ${((global.data.jacoin[event.threadID][event.senderID].workcountdown - time.getTime()) / 1000)} giây`, event.threadID, event.messageID);
    } else {
        ...
        api.sendMessage(`Bạn đã làm việc và có ${workgetmoney} SOcoin`, event.threadID ,event.messageID);
...
async function totalcoin (event,api) {
    api.sendMessage(`Bạn có ${global.data.jacoin[event.threadID][event.senderID].totalcoin} SOcoin`, event.threadID, event.messageID);
}
    ...
    var to = msg * cmNumber;
        ...
        var out = sort.map(e => ({
          ...
          for (var i = from; i < to; i++) {
            if(out[i] != undefined){
            sus += `${i+1}. ${(await api.getUserInfo(out[i].id))[out[i].id].name} : ${out[i].coin} SOCOIN\n`
            }
          }
