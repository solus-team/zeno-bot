...
if ( event.logMessageType == "log:subscribe" ){
	if (event.logMessageData.addedParticipants.some(id => id.userFbId == api.getCurrentUserID())) {
		return api.sendMessage(`> Solus đã kết nối! Dấu lệnh : ${global.config.prefix}`, event.threadID, () => {
			api.changeNickname(`${global.config.botname}`, event.threadID, api.getCurrentUserID());
		});
...
async function checkupdate() {
	try {
		const { data } = await axios.get("https://raw.githubusercontent.com/HieuNe1703/solus-bot/master/package.json");
